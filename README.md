...
- ✅ **رفع ملفات كبيرة**: يدعم رفع ملفات حتى **2000 MiB** (سقف تلجرام الصلب للبوتات، لا يُرفع بإعداد)، وفيديوهات طويلة (3 ساعات+)
- ✅ **تصغير قبل الرفض** 🗜️ (**للأدمن**): مقطع 4K تجاوز السقف بحجمه الخام (VP9/AV1) يُحوَّل أولاً إلى H.264 — وهو ما يقلّص الحجم للنصف تقريباً — ثم يُعاد فحصه، بدل رفضه فوراً
- ✅ **جودات متعددة**: العضو يختار فيديو (1080p) أو صوت (MP3)، و**الأدمن وحده** يرى أزرار الجودة كاملة (أعلى جودة متاحة بلا سقف / 1080p / 720p / 480p / 360p / MP3)
- ✅ **أعلى جودة للأدمن** 🏆: زرّ «أعلى جودة متاحة» ينزّل 1440p/4K فأعلى بلا سقف دقة أو ترميز (يوتيوب لا يوفّر فوق 1080p إلا بـ VP9/AV1)، ويُعاد ترميزه تلقائياً إلى H.264 قبل الرفع
...
# اختياري: دفع القياسات الحقيقية إلى مركز التحكم (معطّل افتراضياً)
ADMIN_TELEMETRY_ENABLED=0
ADMIN_TELEMETRY_URL=
ADMIN_TELEMETRY_SECRET_HEX=       # سر عشوائي مستقل: 32 بايت / 64 رمز hex
...
ضع رابط HTTPS المنشور لمركز التحكم. عند ضبطه يظهر اختصار **🎛 مركز التحكم**
للأدمن وحده في المحادثة الخاصة. عند ضغط الاختصار يرسل البوت رسالةً فيها زر
Web App داخلي؛ هذا هو المسار الذي يزوّد اللوحة بهوية Telegram الموقّعة.
ترك الرابط فارغاً أو وضع رابط غير صالح يبقي القائمة الحالية كما هي تماماً.
...
### القياسات الحقيقية (اختياري)

لا يفتح البوت منفذاً على Raspberry Pi. عند ضبط `ADMIN_TELEMETRY_ENABLED=1`
يرسل كل نحو 20 ثانية snapshot صغيراً إلى `ADMIN_TELEMETRY_URL` عبر HTTPS،
موقّعاً بـHMAC باستخدام `ADMIN_TELEMETRY_SECRET_HEX`. استخدم سراً عشوائياً
مستقلاً بطول 32 بايت ولا تستخدم `BOT_TOKEN`. لا تتضمن الحمولة روابط المصدر،
ملفات الوسائط، مسارات محلية، `file_id`، cookies، tokens أو tracebacks.
...
السكربت `update_ytdlp.sh` المرفق يتكفّل بذلك (يحدّث أيضاً `gallery-dl`
و`curl_cffi`):
...
- يرسل **إشعار تلجرام للأدمن** بنتيجة كل تحديث (نجاح/فشل) مع قائمة ما تغيّر
