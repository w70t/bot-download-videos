...
## 3. Project structure
- `bot.py` — main application and entry point (~6,575 lines): Pyrogram handlers, commands (e.g. `/start`, `/cookies`, `/backup`, `/update`, `/uncache`, `/history`, `/dlstats`, `/blockacc`, `/ban`/`/unban`, `/snaptest`), download orchestration, upload, caching, member/admin flows.
